...
describe('calculateLineVat', () => {
  ...
  test('handles decimal base amounts', () => {
    ...
    expect(result.vatAmount).toBe(12.35); // 123.45 * 0.10 = 12.345 → 12.35
    expect(result.totalAmount).toBe(135.8); // 123.45 + 12.35 = 135.80
  });
});
...
describe('validateInvoice', () => {
  ...
  test('mixed VAT rates: %1 + %10 + %20', () => {
    ...
    // Manual: 100*0.01=1, 200*0.10=20, 300*0.20=60 → total = 101+220+360 = 681
    const result = validateInvoice(rawItems, 681);
    expect(result.validation.status).toBe('verified');
    expect(result.totals.grandTotal).toBe(681);
  });

  test('detects mismatch when AI misreads a digit', () => {
    const rawItems = [
      { description: 'Ürün', baseAmount: 800, vatRate: 20, quantity: 1 },
    ];
    // AI read 800 but actual base was 8000 → actual total would be 9600
    // 800 * 0.20 = 160, total = 960, but reported = 9600
  ...
  test('handles rounding within tolerance', () => {
    ...
    // 33.33*0.1=3.333→3.33, 66.67*0.1=6.667→6.67
    // totals: 36.66 + 73.34 = 110.00
